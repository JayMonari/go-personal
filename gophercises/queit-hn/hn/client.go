...
// NewClient returns a client with the HN API baseURL if none is provided.
...
func (c *Client) TopItems() (ids []int, err error) {
...
// GetItem returns the Item defined by the provided ID.
...
//
// For the purpose of this exercise, we only care about items where the type is
// "story", and the URL is set.
type Item struct {
	By          string `json:"by"`
